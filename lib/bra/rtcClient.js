...
//#region Methods
    ...
    let onClose = function( event )
    {
        ...
        console.log("%c" + "disconnected" + "%o", DEBUG_STYLE, url);

        stopKeepAlive();
        emit(RTCEvent.ClientDisconnected, { url: url });
    };
...
//#region EventHandlers
    ...
    let handleCallAccepted = function( event )
    {
        ...
        // Create a data channel to avoid renegotiation.
    ...
    let handleRemoteOffer = function( event )
    {
        ...
        // TODO: Uncaught (in promise) DOMException: Illegal audio/video payload type "65" at level 0
        // TODO: Uncaught (in promise) DOMException: No remoteDescription.
        // TODO: Uncaught (in promise) DOMException: Cannot create answer in stable adapter.js:2170
...
//#region Methods
    ...
    let bind = function( peerConnection, isInitiator )
    {
        ...
        // Handle data channel events internally to track their lifecycle.
        connection.addEventListener("datachannel", function( event )
        {
            channels[event.channel.label] = event.channel;

            event.channel.addEventListener("close", function( other )
            {
                if( other.channel !== undefined )
                {
                    delete channels[other.channel.label];
                }
            });
        });
    ...
    let sendMessage = function( message )
    {
        ...
        if( !channel )
        {
            channel = connection.createDataChannel(label);
    ...
    /**
     * Set a callback to message events.
     */
    let onMessage = function( callback )
    {
        const label = "message";
