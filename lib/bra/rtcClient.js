...
//#region Methods
    ...
    {
        startKeepAlive();
        console.log("%c" + RTCEvent.ClientConnected + "%o", DEBUG_STYLE, socket.url);
    ...
    let onClose = function( event )
    {
        userId = null;
        stopKeepAlive();
        console.log("%c" + RTCEvent.ClientDisconnected + "%o", DEBUG_STYLE, socket.url);
    ...
    let onMessage = function( msg )
    {
        ...
        console.log("%c%s" + "%o", DEBUG_STYLE, message.id, msg.data);
    ...
    let sendMessage = function( message, response )
    {
        ...
        if( message.id !== "ping" )
        {
            console.log("%c%s" + "%o", DEBUG_STYLE, message.id, msg);
        }
     ...
     * Call a user.
     */
    let call = function( callId, calleeId, response )
    {
        if( userId === calleeId )
        {
...
//#region EventHandlers
    ...
    let handleCallResponse = function( event )
    {
        ...
        {
            // Create the call.
            let call = new RTCCall(event.callId, event.callerId, event.calleeId, JSON.parse(event.iceServers));
...
export function RTCCall( callId, callerId, calleeId, iceServers )
{
//#region Variables
    let connection = null;
    let candidates = [];
